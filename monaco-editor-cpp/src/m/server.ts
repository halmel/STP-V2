...
import MatlabLifecycleManager, {
  MatlabConnectionStatusParam,
} from './lifecycle/MatlabLifecycleManager';
...
import ExecuteCommandProvider, {
  MatlabLSCommands,
} from './providers/lspCommands/ExecuteCommandProvider';
import NavigationSupportProvider, {
  RequestType,
} from './providers/navigation/NavigationSupportProvider';
...
const documentManager: TextDocuments<TextDocument> = new TextDocuments(
  TextDocument,
);
...
MatlabLifecycleManager.addMatlabLifecycleListener((error, lifecycleEvent) => {
  if (error != null) {
    Logger.error(
      `MATLAB Lifecycle Error: ${error.message}\n${error.stack ?? ''}`,
    );
  }
...
//   startMatlabIfOnStartLaunch();
});

async function startMatlabIfOnStartLaunch(): Promise<void> {
  // Launch MATLAB if it should be launched early
  const connectionTiming = (await ConfigurationManager.getConfiguration())
    .matlabConnectionTiming;
  const a = await ConfigurationManager.getConfiguration();
  console.log(a);
...
NotificationService.registerNotificationListener(
  ...
  (data) =>
    MatlabLifecycleManager.handleConnectionStatusChange(
      data as MatlabConnectionStatusParam,
    ),
);
...
//   }
LintingSupportProvider.lintDocument(params.document, connection);
});

// Handle execute command requests
// connection.onExecuteCommand((params) => {
//   void ExecuteCommandProvider.handleExecuteCommand(
//     params,
//     documentManager,
//     connection,
//   );
...
connection.onCompletion(async (params) => {
  ...
  return await CompletionProvider.handleCompletionRequest(
    params,
    documentManager,
  );
});
...
connection.onSignatureHelp(async (params) => {
  ...
  return await CompletionProvider.handleSignatureHelpRequest(
    params,
    documentManager,
  );
});
...
//   return await FormatSupportProvider.handleDocumentFormatRequest(
//     params,
//     documentManager,
//   );
// });
...
//   return await NavigationSupportProvider.handleDefOrRefRequest(
//     params,
//     documentManager,
...
connection.onDocumentSymbol(async (params) => {
  return await NavigationSupportProvider.handleDocumentSymbol(
    params,
    documentManager,
